...
class ChangePointDetector:
    ...
    def __init__(self, min_train: int = 60, refit_every: int = 21) -> None:
        self.min_train = min_train
        self.refit_every = refit_every

    def classify(self, df: pd.DataFrame) -> pd.Series:
        """Label each day by the trend/vol character of the most recent
        segment found by a PELT fit on the expanding window up to the last
        refit point at or before that day.
        ...
        for i, refit_at in enumerate(refit_points):
            segment_label = self._label_last_segment(daily_return.iloc[: refit_at + 1])
